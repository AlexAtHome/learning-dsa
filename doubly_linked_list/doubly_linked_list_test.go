...
import (
	...
	dll "alexalex.dev/dsa/doubly_linked_list"
)
...
func TestNodeNext(t *testing.T) {
	...
	if actual.Value != bar.Value {
		t.Fatalf("Expected %v instead of %v to be next to %v in %v", bar, actual, list.GetHead(), list)
	}
}
...
func TestNodePrev(t *testing.T) {
	...
	baz := list.Insert("baz")
	fmt.Println(list)
	if baz.Prev.Value != bar.Value {
		t.Fatalf("Expected %v to be the previous to %v in %v", bar, baz, list)
	}
}

func TestNodePointingAtItself(t *testing.T) {
	list := dll.DoublyLinkedList{}
	list.Insert("foo")
	actual := list.GetHead().Next
	if actual != nil {
